...
def create_emotions_df(input_path):
	...
	emotions_df = (emotions_df.set_index("level_1").rename_axis("Milliseconds"))
	emotions_df.drop(emotions_df.columns[0:9], inplace = True, axis = 1)

	return emotions_df

def plot_emotions(emotions_df, output_path):
	'''plots data from the emotions dataframe'''
	prev_feature = 0
...
def create_power_hr_df(hr_input_path, pwr_input_path):
	...
	heart_rate_df = pd.read_csv(hr_input_path, sep = "-", 
		names = ["capture", "HR"])
	heart_rate_df["capture"] = heart_rate_df["capture"].str.extract(r'(\d{1,4})')

	# load power df
	power_df = pd.read_csv(pwr_input_path, sep = "-", 
		names = ["capture", "Power"])
	power_df["capture"] = power_df["capture"].str.extract(r'(\d{1,4})')

	# merge power and heart rate together
	merged_df = (heart_rate_df.merge(power_df, on = "capture").
		set_index("capture").rename_axis("seconds"))
	return merged_df

def clean_power_hr_df(hr_input_path, pwr_input_path):
	merged_df = create_power_hr_df(hr_input_path, pwr_input_path)

	# get rid of any unnecessary whitespace including between digits
...
def plot_tesseract_results(cleaned_df, output_path):
	...
	fig.suptitle("Seuil 30", x = 0.5155, y = 0.99)
...
def generate_subject(emotions_df, power_hr_df):
	...
	new_pwr_df = (power_hr_df.groupby(power_hr_df.index).mean().
		drop(["ewm_HR", "ewm_Power"], axis = 1).rename_axis("group_secs"))

	subject = new_pwr_df.merge(new_emotions_df, on = "group_secs")
	return subject
...
def even_more_plots(subject):
	...
	plt.text(0.9, 0.05, "r = {:.3f}".format(correlation), transform = plt.gca().transAxes, 
		horizontalalignment = "center", verticalalignment = "center", style = "italic")
	plt.suptitle("Seuil 30: Valence vs. Power (1 min increments)", 
		x = 0.5155, y = 0.95)
	...
	plt.suptitle("Seuil 30: Pearson Correlations", 
		x = 0.5155, y = 0.95)

	# generate plot of engagement and attention over time
	plt.figure(3)
	subject[["Engagement", "Attention"]].plot(color = {"green", "black"})
	plt.xlabel("Minutes", labelpad = 7)
	plt.ylabel("FEA Metric", labelpad = 7)
	plt.suptitle("Seuil 30: Engagement/Attention", 
		x = 0.5155, y = 0.95)
